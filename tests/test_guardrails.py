...
def test_live_mode_is_blocked(monkeypatch):
    ...
    with pytest_raises(LiveModeBlockedError):
        assert_simulation_mode()
...
class pytest_raises:
    def __init__(self, exc):
        import pytest  # local import
        self.pytest = pytest
        self.exc = exc

    def __enter__(self):
        return self.pytest.raises(self.exc).__enter__()

    def __exit__(self, exc_type, exc, tb):
        return self.pytest.raises(self.exc).__exit__(exc_type, exc, tb)
